...
//이 경로는 POST방식으로만 호출이 가능 (파일 등록)
	...
	public String uploadForm(MultipartFile file, ModelAndView mv, HttpSession session, Member member) {
	    ...
	    session.setAttribute("sleepyresult",deltempvidFile(uploadPathvideo)); // 파일 삭제
	...
	public String deltempvidFile(String uploadPath) { // 업로드 된 비디오 개수 조절
		...
		if(fileslist.length>1&&readtxt.equals(fileslist[0])||readtxt.equals(fileslist[1])) { //만약 가장 최신영상과 그 이전 영상이 딥러닝 판독값과 같다면
			 
			result = readtxt; //세션에 play 값 받기
		}
